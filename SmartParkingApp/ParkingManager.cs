...
namespace SmartParkingApp {
    public class ParkingManager
    {
        ...
        public bool tryRegistrate(string userPhone, string userName, string carNumber, string userPassword)
        {
            if (users.Any(a => a.Phone == userPhone))
            return false;
            else
            {
                users.Add(new User { Name = userName, Phone = userPhone, CarPlateNumber = carNumber, Password = userPassword, Id = idCounter()});
                return true;
            }
        }
        ...
        {
            if (users.Any(a => a.Phone == Login & a.Password == Password))
                return true;
            else
            {
                return false;
        ...
        public List<ParkingSession> GetSessionsByUser(string userCarNumber)
        {
            var session = pastSessions.FindAll(a => a.CarPlateNumber == userCarNumber);
            return session;
        }
        public bool CheckActivSessionByUser(string userPhone){
            string userCarNumber = GetCarNumberByPhone(userPhone);
            var session = activeSessions.FirstOrDefault(a => a.CarPlateNumber == userCarNumber);
            if (session == null)
                return false;
            else
                return true;
        ...
        public DateTime GetActivSessionByUser(string userPhone)
        {
            string userCarNumber = GetCarNumberByPhone(userPhone);
            var session = activeSessions.FirstOrDefault(a => a.CarPlateNumber == userCarNumber);
            DateTime _timeEnter = session.EntryDt;
            return _timeEnter;
        }
        public string GetCarNumberByPhone(string Phone)
        {
            var user = users.FirstOrDefault(a => a.Phone == Phone);
            string userCarNumber = user.CarPlateNumber;
            return userCarNumber;
            
        }

        public List<ParkingSession> GetActivSessions()
        {
            return activeSessions;
        }

        public List<ParkingSession> GetPastSessions()
        {
            return pastSessions;
        }

        public double GetPlacesLeftPercentage()
        {
            double _left = 100 - (activeSessions.Count() / parkingCapacity);
            return _left;
        }
        public decimal GetCost(double diffInMinutes) {
